<script lang="ts">
	import { signInWithGoogle, signInWithGithub, signInWithEmail, signInWithFacebook } from '$firebase';
...
<div class=" bg-secondary flex flex-col p-8 w-96 h-fit justify-around self-center rounded-lg gap-4 shadow-xl">
	...
	<form method="POST">
		<div id="toolLogin" class="tooltip tooltip-top tooltip-error before:hidden after:hidden tooltip-open" data-tip="">
			<input bind:value={email} on:blur={blurEmail} type="text" placeholder="Email" class="input w-full max-w-xs " name="email_box" id="email_box"/>
			<input
				bind:value={password}
				type="password"
...
<style>
	.wrapper {
		...
		background-image: radial-gradient(49% 81% at 45% 47%, #bc03ff45 0%, #073aff00 100%),
			radial-gradient(113% 91% at 17% -2%, #1900ffff 1%, #ff000000 99%), radial-gradient(142% 91% at 83% 7%, #e600ffff 1%, #3f00ff00 99%),
			radial-gradient(142% 91% at -6% 74%, #f200ffff 1%, #8900ff00 99%), radial-gradient(142% 91% at 111% 84%, #0016ffff 0%, #ac00ffff 100%);
	}
</style>
